...
static adc_oneshot_unit_handle_t adc_pot_handle;
...
void steering_check(void *arg)
{
    ...
    while (1)
    {
        ESP_ERROR_CHECK(adc_oneshot_read(adc_pot_handle, ADC_CHANNEL_9, &adc_result));
        adc_result = ((adc_result * 100) / 1517) - 135;

        lcd_write_string(0, "test %i", adc_result);
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(adc_oneshot_new_unit(&init_config1, &adc_pot_handle));
    adc_oneshot_chan_cfg_t config = {
        .bitwidth = ADC_BITWIDTH_DEFAULT,
        .atten = ADC_ATTEN_DB_12,
    };
    ESP_ERROR_CHECK(adc_oneshot_config_channel(adc_pot_handle, ADC_CHANNEL_9, &config));
    ...
    lcd_init();
    lcd_write_string(0, "test %i", 1234);
    //    xTaskCreate(ps5_check, "ps5_check", 4096, NULL, 5, NULL);
    xTaskCreate(steering_check, "steering_check", 4096, NULL, 5, NULL);
}
